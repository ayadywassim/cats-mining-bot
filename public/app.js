const API = 'https://cats-mining-ayadi.onrender.com';
...
function openWithdraw() {
  ...
  content.innerHTML = `
    ...
    <div class="modal-row"><div class="modal-label">${T('amount')}</div><input class="modal-input" type="number" id="w-amount" placeholder="2.0" min="2" step="0.1"></div>
    ...
    <div class="modal-fee">${T('fee')}: 5% · Min: 2 TON</div>
...
async function submitWithdraw() {
  ...
  if (!amount||amount<2){toast('⚠ Min 2 TON');return;}
  ...
  try {
    ...
    if (d.success) { toast('✓ '+T('withdrawSubmitted')); document.getElementById('withdraw-modal').style.display='none'; refreshUser(); }
      ...
      else if (d.error==='MIN_AMOUNT') toast('⚠ Min 2 TON');
